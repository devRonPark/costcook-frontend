...
const RecipePage = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/recipes?page=${page}&size=9&sort=${sort}&order=${order}`);
         ...
         (
          <List key={recipe.id}>
              <a href={`http://localhost:8080/api/recipes/${recipe.id}`}>
                ...
                <RecipeImage alt={recipe.title} src={`http://localhost:8080/img/${recipe.thumbnailUrl}.jpg`}/>
                </RecipeImageBox>
              </a>
            <TitleText>{recipe.title}</TitleText>
            <PriceText>{recipe.price}원</PriceText>
            <StarText>{renderStars(recipe.avgRatings)} ({recipe.avgRatings}점)</StarText>
          </List>
         ))
      )}
      <LoadingBox>
        {hasMore ? <p ref={ref}>로딩 중...</p> : <p>더 이상 불러올 데이터가 없습니다.</p>}
      </LoadingBox>
